...
# will be plotted. For the two-dimensional case it is calculated by:
#
# .. math::
#
...
svM = np.sqrt(s[0, 0] ** 2 + s[1, 1] ** 2 - s[0, 0] * s[1, 1] + 3 * s[0, 1] ** 2)
plotter = field.view(point_data={"Cauchy Stress (von Mises)": fem.project(svM, region)})
plotter.plot("Cauchy Stress (von Mises)", show_edges=False).show()
