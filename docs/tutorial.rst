...
.. image:: _static/undeformed_mesh.png
   :width: 600px
...
The default solver of FElupe is SuperLU provided by SciPy. A significantly faster alternative is `pypardiso <https://pypi.org/project/pypardiso/>`_ which can be installed from PyPI with ``pip install pypardiso`` (not included with FElupe). The optional argument ``solver`` of :func:`felupe.solve.solve` accepts a user-defined solver.
...
.. image:: _static/deformed_mesh.png
   :width: 600px