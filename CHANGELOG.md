...
- Add `SolidBodyContact`, a high-level node-to-segment penalty contact between two deformable solid bodies. The two contact surfaces are given by field containers, whose fields are defined on a `RegionBoundary` (e.g. `RegionQuadBoundary` or `RegionHexahedronBoundary`). It supports 2D and 3D, Coulomb friction and an optional symmetric two-pass contact search. A `SolidBodyContact` is supported as an item in a `Step`.
