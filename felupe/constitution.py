...
class GeneralizedMixedFieldStabilized:
    def __init__(self, P, A, param):
        self.param = param[:-1]
        self.eps = param[-1]
        self.fun_P = P
    ...
    def f_u(self, F, p, J):
        ...
        return self.P + (self.Pbb - self.PbbF / 3 * self.iFT) + p * self.detF * self.iFT
    ...
    def f(self, F, p, J):
        ...
        self.iFT = transpose(inv(F))
        self.Fb = (J / self.detF) ** (1 / 3) * F
        self.P = self.fun_P(F, (0, self.eps * self.param[1]))
        self.Pb = self.fun_P(self.Fb, (self.param[0], self.param[1] * (1 - self.eps)))
        self.Pbb = (J / self.detF) ** (1 / 3) * self.Pb
        self.PbbF = ddot(self.Pbb, F)
    ...
    def A(self, F, p, J):
        ...
        self.eye = identity(F)
        self.P4 = cdya_ik(self.eye, self.eye) - 1 / 3 * dya(F, self.iFT)
        self.A4 = self.fun_A(F, (0, self.eps * self.param[1]))
        self.A4b = self.fun_A(self.Fb, (self.param[0], self.param[1] * (1 - self.eps)))
        self.A4bb = (J / self.detF) ** (2 / 3) * self.A4b
...
class GeneralizedMixedFieldv2:
    ...
    def f_u(self, F, p, J):
        ...
        return (
            self.Pbb
            - self.PbbF / 3 * self.iFT
            + p / 3 * self.detF ** (1 / 3) * self.iFT
        )
    ...
    def A(self, F, p, J):
        ...
        self.P4 = cdya_ik(self.eye, self.eye) - 1 / 3 * dya(F, self.iFT)
        self.A4b = self.fun_A(self.Fb, self.param)
        self.A4bb = J ** 2 / self.detF ** (-2 / 3) * self.A4b
    ...
    def A_uu(self, F, p=None, J=None):
        ...
        A4 = (
            ...
            * (cdya_il(self.iFT, self.iFT) + dya(self.iFT, self.iFT) / 3)
            ...
            / 3
            * self.detF ** (1 / 3)
            * (dya(self.iFT, self.iFT) / 3 - cdya_il(self.iFT, self.iFT))
        )
    ...
    def A_up(self, F, p, J):
        ...
        return self.detF ** (1 / 3) / 3 * self.iFT
    ...
    def A_uJ(self, F, p, J):
        ...
        Ps = self.Pbb - self.PbbF / 3 * self.iFT
        return (-self.FA4bbF / 3 * self.iFT + Ps + ddot(F, self.A4bb)) / J
