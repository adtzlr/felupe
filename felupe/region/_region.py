...
class Region:
    ...
    def __init__(self, mesh, element, quadrature, grad=True):
        ...
        #
        # h_ap
        self.h = np.array([self.element.shape(p) for p in self.quadrature.points]).T
        ...
        # dhdr_aJp
        self.dhdr = np.array(
            [self.element.shape_derivative(p) for p in self.quadrature.points]
        ).transpose(1, 2, 0)

        if self.element.nshape > 1 and self.mesh.dim == self.element.dim and grad:
