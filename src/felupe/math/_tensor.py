...
def det(A, out=None):
    ...
    if A.shape[:2] == (3, 3):
        tmp = np.prod([A[0, 0], A[1, 1], A[2, 2]], axis=0)
        np.add(detA, tmp, out=detA)

        np.prod([A[0, 1], A[1, 2], A[2, 0]], axis=0, out=tmp)
        np.add(detA, tmp, out=detA)

        np.prod([A[0, 2], A[1, 0], A[2, 1]], axis=0, out=tmp)
        np.add(detA, tmp, out=detA)

        np.prod([A[2, 0], A[1, 1], A[0, 2]], axis=0, out=tmp)
        np.add(detA, -tmp, out=detA)

        np.prod([A[2, 1], A[1, 2], A[0, 0]], axis=0, out=tmp)
        np.add(detA, -tmp, out=detA)

        np.prod([A[2, 2], A[1, 0], A[0, 1]], axis=0, out=tmp)
        np.add(detA, -tmp, out=detA)

    elif A.shape[:2] == (2, 2):
        tmp = np.prod([A[0, 0], A[1, 1]], axis=0)
        np.add(detA, tmp, out=detA)

        np.prod([A[0, 1], A[1, 0]], axis=0, out=tmp)
        np.add(detA, -tmp, out=detA)
