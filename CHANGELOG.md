...
- Add `MaterialViewIncompressible(umat)` to view force-stretch curves for incompressible uniaxial tension/compression, planar shear and equi-biaxial tension.
- Add `view = Hyperelastic.view()` to invoke `view = ViewMaterialIncompressible(Hyperelastic)` as method and `ax = Hyperelastic.plot()` for `ax = ViewMaterialIncompressible(Hyperelastic).plot()` with alias name `imshow()` and a `screenshot(filename="umat.png")`-method.
